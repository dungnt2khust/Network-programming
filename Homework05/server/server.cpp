...
void resolveRequest(char* rcvBuff, SOCKET &connectedSocket, string &accountUser, string &clientInfor, bool &isLogin);
void loginServer(string data, SOCKET &connectedSocket, string &accountUser, string &clientInfor, bool &isLogin);
void receiveFromClient(string data, SOCKET &connectedSocket, string &accountUser, string &clientInfor, bool &isLogin);
void logOutServer(string data, SOCKET &connectedSocket, string &accountUser, string &clientInfor, bool &isLogin);
string getCurrentTime();
void writeToLog(string method, string message, string resultcode, string clientInfor);
void readAccountFile();
int Receive(SOCKET s, char *buff, int size, int flags, string &accountUser, string &clientInfor, bool &isLogin);
...
LRESULT CALLBACK windowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	readAccountFile();
	static wchar_t messages[] = L"Connected";
	switch (message) {
	case WM_SOCKET:
	{
		...
		case FD_READ:
		{
			...
			if (ret > 0) {
				...
				rcvBuff[ret] = 0;	
				isConnect = true;
				resolveRequest(rcvBuff, client[i], accountName[i], clientIn4[i], isLogin[i]);
			}
		}
		break;
...
void resolveRequest(char* rcvBuff, SOCKET &connectedSocket, string &accountUser, string &clientInfor, bool &isLogin) {
	...
	switch (stoi(choose)) {
	case 1:
		loginServer(data, connectedSocket, accountUser, clientInfor, isLogin);
		break;
	case 2:
		receiveFromClient(data, connectedSocket, accountUser, clientInfor, isLogin);
		break;
	case 3:
		logOutServer(data, connectedSocket, accountUser, clientInfor, isLogin);
		break;
	default:
		break;
	}
